...
const Profile = () => {
  ...
  const [followeingsLimit, setFolloweingsLimit] = useState(3);

  // SWR
  const { data: followersData, error: followerError } = useSWR(`http://localhost:3065/user/followers?limit=${followersLimit}`, fetcher);
  const { data: followingsData, error: followingError } = useSWR(`http://localhost:3065/user/followings?limit=${followeingsLimit}`, fetcher);
  ...
  const loadMoreFollowings = useCallback(( )=> {
    setFolloweingsLimit((prev) => prev + 3);
  }, []);
